...
function createCard(spec, index) {
  ...
  card.innerHTML = `
    <div class="friend-post-author">
      <a href="${spec.authorUrl || '#'}" target="_blank" rel="noopener noreferrer" class="friend-author-link">
        <img src="${spec.logo || ''}" 
             alt="${spec.author || '未知作者'}" 
             class="friend-author-avatar"
             onerror="this.style.display='none'">
        <div class="friend-author-info">
          <span class="friend-author-name">${spec.author || '未知作者'}</span>
          <time class="friend-post-time">${formattedDate}</time>
        </div>
      </a>
    </div>
    <div class="friend-post-content">
      <h3 class="friend-post-title">${spec.title || '无标题'}</h3>
      <p class="friend-post-desc">${spec.description || '暂无描述'}</p>
    </div>
    <a href="${spec.postLink || '#'}" target="_blank" rel="noopener noreferrer" class="friend-post-arrow">
